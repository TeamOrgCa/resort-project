...
const parsePayload = (value: unknown): RejectReschedulePayload | null => {
  if (typeof value !== "object" || value === null) {
    return null;
  }

  const payload = value as Partial<RejectReschedulePayload>;

  if (
    typeof payload.rescheduleId !== "string" ||
    !payload.rescheduleId.trim() ||
  ...
  return {
    rescheduleId: payload.rescheduleId.trim(),
...
export async function POST(request: Request) {
  try {
    ...
    const { data: requestRow, error: requestError } = await staffContext.supabase
      ...
      .select("reschedule_id, reservation_id, status")
    ...
    const { error: updateReservationError } = await staffContext.supabase
      .from("reservations")
      .update({ status: nextReservationStatus })
      .eq("reservation_id", requestRow.reservation_id);

    if (updateReservationError) {
      return NextResponse.json(
        { success: false, message: "Failed to restore reservation status." },
        { status: 500 }
      );
    }
    ...
    if (updateRequestError) {
      return NextResponse.json(
        { success: false, message: "Reservation restored but reschedule request could not be marked rejected." },
        { status: 500 }
      );
    }

    const auditSuccess = await createAuditLog(staffContext, {
      action: `Rejected reschedule request (${payload.rejectionReason})`,
    ...
    return NextResponse.json(
      { success: false, message: "Unexpected error while rejecting reschedule request." },
      { status: 500 }
    );
  }
}